    ...
    var Controller = {
        index: function () {
            ...
            Table.api.init({
                search: false,
                extend: {
                    index_url: 'userflux/flux/index',
                    add_url: 'userflux/flux/add',
                    edit_url: '',
                    del_url: 'userflux/flux/del',
                    multi_url: 'userflux/flux/multi',
                    table: 'userflux',
                }
            });
            ...
            table.bootstrapTable({
                ...
                columns: [
                    [
                        ...
                        {field: 'logintime', title: __('Logintime'), operate: false},
                        {field: 'jointime', title: __('Jointime'), operate: false},
                        {field: 'num', title:'未审核' , operate: false},
                        {field: 'from',title: __('from'),operate: 'LIKE %...%', searchList: $.getJSON('userflux/flux/getfrom?search=1')},
                        {field: 'auth',title: __('from'),operate: 'LIKE %...%', searchList: $.getJSON('userflux/flux/getfrom?search=2')},
            ...
            events: {//绑定事件的方法
                ...
                operate: $.extend({
                    'click .btn-detail': function (e, value, row, index) {
                        e.stopPropagation();
                        Backend.api.open('userflux/flux/detail/ids/' + row['uid'], __('Detail'));
                    }
