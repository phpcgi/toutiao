...
load_trait('libray/User');
load_trait('libray/Userflux');
use app\apiweb\library\Userflux;
use app\apiweb\library\User;
class Api extends Rest
{
    ...
    public static  function reg($phone,$data,$type='', $keeptime = 0){
        if(!$data)
        {
            return false;
        }
        ...
        $emailold = UsersOld::get(['email'=>$data['email']]);
        if ($admin)
        {
            return Rjson('',-1,'手机号已存在');
        }
        if ($email)
        {
            return Rjson('',-1,'邮箱已存在');
        }
        if(!$type){
            if ($emailold)
            {
    ...
    {
        $admin = usermodel::get(['phone' => $phone]);
        $email = usermodel::get(['email' => $phone]);
        if (!$admin && !$email)
        {
            return Rjson('',-1,'账号不存在');
        }
        if($email){
            $admin =  usermodel::get(['phone' => $email->phone]);
        }
        if($admin->status==0){
            return Rjson('',-1,'您申请的账号正在审核中，请耐心等待');
        }
