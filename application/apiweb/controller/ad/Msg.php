...
class Msg extends Api
{
    ...
    */
    public function index(){
        ...
        $msginfo =collection(msgs::all(['type'=>2]))->toArray();
        ...
        $msginfo =collection(Msgtem::all(['type'=>2,'uid'=>$username->uid]))->toArray();
    ...
    public function NoRead(){
        $username = Request::instance()->session('userinfo');
        $msginfo =collection(Msgtem::all(['type'=>2,'uid'=>$username->uid,'status'=>0]))->toArray();
        $data = self::_getinfo($msginfo);
        return Rjson($data);
    }
