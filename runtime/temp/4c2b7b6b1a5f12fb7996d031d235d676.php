<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:61:"D:\toutiao\public/../application/index\view\toutiao\anli.html";i:1515128426;}*/ ?>
<!DOCTYPE html>
<html>
	<head>
		<meta charset="UTF-8">
		<link rel="stylesheet" href="__CDN__/assets/css/index/anli.css" />
		<link rel="stylesheet" href="__CDN__/assets/css/index/icommon.css" />
		<link rel="stylesheet" type="text/css" href="__CDN__/assets/css/index/pagination.css" media="screen">
		<title></title>
	</head>
	<body>
		<div id="container">
		<div id="nav">
				<div class="nav_l n_conmment">
					<img src="__CDN__/assets/img/shouye/logo1.png"/>
				</div>
				<div class="nav_c n_conmment">
					<ul class="nav_cc">
						<li class="zhuye">首页</li>
						<li class="bangd">榜单</li>
						<li class="xmtxy">新媒体学院</li>
						<li class="chanpin">产品</li>
						<li class="gywm">关于我们</li>
						<li class="anl  se">成功案例</li>
					</ul>
					<div class="z_nav">
						<div class="z_top">
							<img src="__CDN__/assets/img/shouye/xtsj.png" />
						</div>
						<div class="z_bottom">
							<p class="tout tw1 ttb">头条榜</p>
							<p class="baow tw1 bwb">爆文榜</p>
						</div>
					</div>
					<div class="z_nav2">
						<div class="z_top">
							<img src="__CDN__/assets/img/shouye/xtsj.png" />
						</div>
						<div class="z_bottom">
							<p class="tout tw1">公司介绍</p>
							<p class="baow tw1">豹纹榜</p>
							<p class="tout tw1">头条榜</p>
							<p class="baow tw1">豹纹榜</p>
							<p class="tout tw1">头条榜</p>
							<p class="baow tw1">豹纹榜</p>
						</div>
					</div>
					<?php if($name): ?>
					<!--已登陆状态-->
					<div class="z_nav5">
						<div class="z_top5">
							<img src="__CDN__/assets/img/shouye/xtsj.png" />
						</div>
						<div class="z_bottom5">
							<p class="tout tw15 ggz">返回我的主页</p>
							<!--
							<p class="tout tw15 ggz">进入广告组</p>
							<p class="baow tw15 llz">进入流量组</p>-->
							<p class="tout tw15 tuichu">退出登录</p>
						</div>
					</div>
					<!--未登陆状态-->
					<?php else: ?>
					<div class="z_nav3">
						<div class="z_top">
							<img src="__CDN__/assets/img/shouye/xtsj.png" />
						</div>
						<div class="z_bottom">
							<!--<p class="tout tw1">返回我的主页</p>-->
							<p class="baow tw1 ggzdl">广告主登录</p>
							<p class="tout tw1 llzdl">流量主登录</p>
						</div>
					</div>
					<?php endif; ?>
				</div>
				<div class="nav_r n_conmment">
					<?php if($name): ?>
					<p class="deng"><?php echo $name; ?></p>
					<?php else: ?>
					<p class="deng">登录</p>
					<?php endif; ?>
					<p class="shu1">|</p>
					<p class="zhu">注册</p>
					<div class="z_nav4">
						<div class="z_top4">
							<img src="__CDN__/assets/img/shouye/xtsj.png" />
						</div>
						<div class="z_bottom4">
							<p class="tout tw1 ggzzc">流量主注册</p>
							<p class="baow tw1 llzzc">广告主注册</p>
							<!--<p class="tout tw1">退出登陆</p>-->
						</div>
					</div>
				</div>
			</div>
			</div>
		<ul class="table">
			<p class="shu">|</p>
			<li>电影宣传</li>
			<li>电影宣传</li>
			<li>电影宣传</li>
			<li>电影宣传</li>
		</ul>
		<div class="twk">
			<!--<div class="pic">
				<div class="pic_l">
					<img src="__CDN__/assets/img/xmtxy/huan_1.png" />
				</div>
				<div class="pic_r">
					<div class="titl">英菲尼迪汽车新闻检测报告</div>
					<p class="nei">啊看你看你看淡看可那都是福克纳苏丹诺夫咖喱啊大沙发看了那可是那地方卢卡斯啊但是陆阔发生陆阔费劲啊老师阿斯顿发了卡积分</p>
					<p class="cha">查看详情</p>
				</div>
			</div>
			<div class="pic">
				<div class="pic_l">
					<img src="__CDN__/assets/img/xmtxy/huan_1.png" />
				</div>
				<div class="pic_r">
					<div class="titl">英菲尼迪汽车新闻检测报告</div>
					<p class="nei">啊看你看你看淡看可那都是福克纳苏丹诺夫咖喱啊大沙发看了那可是那地方卢卡斯啊但是陆阔发生陆阔费劲啊老师阿斯顿发了卡积分</p>
					<p class="cha">查看详情</p>
				</div>
			</div>
			<div class="pic">
				<div class="pic_l">
					<img src="__CDN__/assets/img/xmtxy/huan_1.png" />
				</div>
				<div class="pic_r">
					<div class="titl">英菲尼迪汽车新闻检测报告</div>
					<p class="nei">啊看你看你看淡看可那都是福克纳苏丹诺夫咖喱啊大沙发看了那可是那地方卢卡斯啊但是陆阔发生陆阔费劲啊老师阿斯顿发了卡积分</p>
					<p class="cha">查看详情</p>
				</div>
			</div>
			<div class="pic">
				<div class="pic_l">
					<img src="__CDN__/assets/img/xmtxy/huan_1.png" />
				</div>
				<div class="pic_r">
					<div class="titl">英菲尼迪汽车新闻检测报告</div>
					<p class="nei">啊看你看你看淡看可那都是福克纳苏丹诺夫咖喱啊大沙发看了那可是那地方卢卡斯啊但是陆阔发生陆阔费劲啊老师阿斯顿发了卡积分</p>
					<p class="cha">查看详情</p>
				</div>
			</div>
			<div class="pic">
				<div class="pic_l">
					<img src="__CDN__/assets/img/xmtxy/huan_1.png" />
				</div>
				<div class="pic_r">
					<div class="titl">英菲尼迪汽车新闻检测报告</div>
					<p class="nei">啊看你看你看淡看可那都是福克纳苏丹诺夫咖喱啊大沙发看了那可是那地方卢卡斯啊但是陆阔发生陆阔费劲啊老师阿斯顿发了卡积分</p>
					<p class="cha">查看详情</p>
				</div>
			</div>
			<div class="pic">
				<div class="pic_l">
					<img src="__CDN__/assets/img/xmtxy/huan_1.png" />
				</div>
				<div class="pic_r">
					<div class="titl">英菲尼迪汽车新闻检测报告</div>
					<p class="nei">啊看你看你看淡看可那都是福克纳苏丹诺夫咖喱啊大沙发看了那可是那地方卢卡斯啊但是陆阔发生陆阔费劲啊老师阿斯顿发了卡积分</p>
					<p class="cha">查看详情</p>
				</div>
			</div>-->
		</div>
		<div class="f_kuang">
			<div class="M-box1"></div>
		</div>
		<div id="bot">
			<div class="bot">北京头条易科技有限公司 | 京ICP备16042456号-1</div>
		</div>
		<div class="yin" style="display: none;"></div>
	</body>
	<script src="https://cdn.bootcss.com/jquery/3.2.1/jquery.js"></script>
	<script src="__CDN__/assets/js/index/js/jquery.pagination.js"></script>
	<script type="text/javascript" src="__CDN__/assets/js/index/js/anli.js" ></script>
	<script type="text/javascript" src="__CDN__/assets/js/index/js/shouye.js" ></script>

</html>
